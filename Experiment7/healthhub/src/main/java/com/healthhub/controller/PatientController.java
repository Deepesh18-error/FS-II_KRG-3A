...
public class PatientController {
    ...
    // Optimized endpoint fetching relationships eagerly
    @GetMapping("/with-appointments")
    public List<PatientDTO> getAllWithAppointments(){
        return service.getAllPatientsWithAppointments();
    }

    // Cursor-based pagination endpoint
    @GetMapping("/cursor")
    public List<PatientDTO> getByCursor(
            @RequestParam(defaultValue = "0") Long lastId,
            @RequestParam(defaultValue = "10") int limit) {
        return service.getPatientsCursor(lastId, limit);
    }
